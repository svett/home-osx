...
function check(filename, goConfig) {
    ...
    if (!!goConfig['buildOnSave']) {
        ...
        let buildPromise = new Promise((resolve, reject) => {
            ...
            fs.readFile(filename, 'utf8', (err, data) => {
                ...
                runTool(args, cwd, 'error', true, null, env, true).then(result => resolve(result), err => reject(err));
            });
        });
    ...
    if (!!goConfig['lintOnSave']) {
        ...
        runningToolsPromises.push(runTool(args, cwd, 'warning', false, lintTool, env));
    }
    if (!!goConfig['vetOnSave']) {
        let vetFlags = goConfig['vetFlags'] || [];
        runningToolsPromises.push(runTool(['tool', 'vet', ...vetFlags, filename], cwd, 'warning', true, null, env));
    }
    if (!!goConfig['coverOnSave']) {
