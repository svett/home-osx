...
function onChangeConfiguration(context, event) {
    // TODO implements
...
function onFolderAdded(context, folder) {
    ...
    reactNativeProjectHelper_1.ReactNativeProjectHelper.getReactNativeVersion(rootPath)
        .then(version => {
        if (version && isSupportedVersion(version)) {
            entryPointHandler.runFunction("debugger.setupLauncherStub", errorHelper_1.ErrorHelper.getInternalError(internalErrorCode_1.InternalErrorCode.DebuggerStubLauncherFailed), () => {
                ...
                return setupAndDispose(reactDirManager, context)
                    .then(() => {
                    let projectRootPath = settingsHelper_1.SettingsHelper.getReactNativeProjectRoot(folder.uri.fsPath);
                    ...
                    let extensionServer = new extensionServer_1.ExtensionServer(rootPath, packager);
