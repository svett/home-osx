...
class EasyMotion {
    ...
    updateDecorations() {
        ...
        const fontWeight = vscode.workspace.getConfiguration().get("editor.fontWeight");
        // Compute font color based on background (remove opacity)
        var Color = require('color');
        let backgroundColor = Color.rgb(configuration_1.Configuration.searchHighlightColor).alpha(1.0);
        let fontColor = "white";
        ...
        for (var i = 0; i < this.markers.length; i++) {
            ...
            if (!configuration_1.Configuration.easymotionChangeBackgroundColor) {
            ...
            this.decorations[keystroke.length].push({
                ...
                renderOptions: {
                    dark: {
                        after: {
                            contentIconPath: EasyMotion.getSvgDataUri(keystroke, backgroundColor.string(), fontFamily, fontColor, fontSize, fontWeight)
                        }
                    },
                    light: {
                        after: {
