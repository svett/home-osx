...
let CommandSurroundAddToReplacement = CommandSurroundAddToReplacement_1 = class CommandSurroundAddToReplacement extends actions_1.BaseCommand {
    ...
    static TryToExecuteSurround(vimState, position) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let endDeleteRange;
            const quoteMatches = [
                { char: "'", movement: () => new motion_1.MoveASingleQuotes() },
                { char: '"', movement: () => new motion_1.MoveADoubleQuotes() },
                { char: "`", movement: () => new motion_1.MoveABacktick() },
            ];
            ...
            const pairedMatchings = [
                { open: "{", close: "}", movement: () => new motion_1.MoveACurlyBrace() },
                { open: "[", close: "]", movement: () => new motion_1.MoveASquareBracket() },
                { open: "(", close: ")", movement: () => new motion_1.MoveAParentheses() },
                { open: "<", close: ">", movement: () => new motion_1.MoveACaret() },
            ];
            ...
            if (target === "t") {
                let { start, stop, failed } = yield new motion_1.MoveAroundTag().execAction(position, vimState);
                let tagEnd = yield new motion_1.MoveInsideTag().execAction(position, vimState);
