...
class RemoteAttachPicker {
    ...
    static ShowAttachEntries(args) {
        ...
        if (!args.pipeTransport || !args.pipeTransport.debuggerPath) {
            ...
            let quoteArgs = args.pipeTransport.quoteArgs != null ? args.pipeTransport.quoteArgs : true; // default value is true
            ...
            return RemoteAttachPicker.ValidateAndFixPipeProgram(pipeProgram).then(pipeProgram => {
                let pipeCmdList = [];
                let scriptShellCmd = "sh -s";
                pipeCmdList.push(pipeProgram);
                const debuggerCommandString = "${debuggerCommand}";
                ...
                let pipeCmd = quoteArgs ? this.createArgumentList(pipeCmdList) : pipeCmdList.join(' ');
