...
var GoSignatureHelpProvider = (function () {
    function GoSignatureHelpProvider(toolForDocs) {
    ...
    GoSignatureHelpProvider.prototype.provideSignatureHelp = function (document, position, token) {
        ...
        return goDeclaration_1.definitionLocation(document, callerPos, this.toolForDocs).then(function (res) {
            if (!res) {
            ...
            if (res.toolUsed === 'godef') {
                ...
                var funcNameStart = declarationText.indexOf(res.name + '('); // Find 'functionname(' to remove anything before it
                if (funcNameStart > 0) {
