    ...
    RubyDebugSession.prototype.stackTraceRequest = function (response, args) {
        ...
        this.rubyProcess.Enqueue('where').then(function (results) {
            ...
            results.forEach(function (stack) {
                var filePath = _this.convertDebuggerPathToClient(stack.file);
                if (!_this._activeFileData.has(filePath) && fs_1.existsSync(filePath)) {
                    _this._activeFileData.set(filePath, fs_1.readFileSync(filePath, 'utf8').split('\n'));
                }
            });
            response.body = {
                stackFrames: results.filter(function (stack) { return stack.file.indexOf('debug-ide') < 0 && +stack.line > 0; })
                    .map(function (stack) {
                    var filePath = _this.convertDebuggerPathToClient(stack.file);
                    var fileData = _this._activeFileData.get(filePath);
                    var gemFilePaths = filePath.split('/gems/');
                    var gemFilePath = gemFilePaths[gemFilePaths.length - 1];
                    return new vscode_debugadapter_1.StackFrame(+stack.no, fileData ? fileData[+stack.line - 1].trim() : (gemFilePath + ':' + stack.line), fileData ? new vscode_debugadapter_1.Source(path_1.basename(stack.file), filePath) : null, _this.convertDebuggerLineToClient(+stack.line), 0);
                })
            };
