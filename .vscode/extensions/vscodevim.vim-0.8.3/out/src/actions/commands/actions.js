...
let PutCommandVisual = class PutCommandVisual extends BaseCommand {
    ...
    exec(position, vimState, after = false) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (register.registerMode === register_1.RegisterMode.LineWise) {
                let result = yield new operator.DeleteOperator(this.multicursorIndex).run(vimState, start, end.getLeft(), false);
                // to ensure, that the put command nows this is
                // an linewise register insertion in visual mode
                let oldMode = result.currentMode;
                result.currentMode = mode_1.ModeName.Visual;
                result = yield new PutCommand().exec(start, result, true);
            ...
            let result = yield new PutCommand(this.multicursorIndex).exec(start, vimState, true);
            result.currentRegisterMode = isLineWise ? register_1.RegisterMode.LineWise : register_1.RegisterMode.CharacterWise;
            result = yield new operator.YankOperator(this.multicursorIndex).run(result, start, end);
            result.currentRegisterMode = register_1.RegisterMode.CharacterWise;
            result = yield new operator.DeleteOperator(this.multicursorIndex).run(result, start, end.getLeftIfEOL(), false);
            result.currentRegisterMode = register_1.RegisterMode.FigureItOutFromCurrentMode;
            return result;
        });
    }
};
            ...
            for (const { line, end } of position_1.Position.IterateLine(vimState)) {
                if (line.trim() === "") {
                    vimState.recordedState.transformations.push({
                        ...
                        position: new position_1.Position(end.line, 0)
                    });
                }
                vimState.allCursors.push(new range_1.Range(end, end));
            }
...
let ActionDeleteLineVisualMode = class ActionDeleteLineVisualMode extends BaseCommand {
    ...
    exec(position, vimState) {
        return __awaiter(this, void 0, void 0, function* () {
            if (vimState.currentMode === mode_1.ModeName.Visual) {
                return yield new operator.DeleteOperator(this.multicursorIndex)
                    .run(vimState, vimState.cursorStartPosition.getLineBegin(), vimState.cursorPosition.getLineEnd());
            }
            else {
...
let ActionChangeLineVisualMode = class ActionChangeLineVisualMode extends BaseCommand {
    ...
    exec(position, vimState) {
        return __awaiter(this, void 0, void 0, function* () {
            return yield new operator.DeleteOperator(this.multicursorIndex)
                .run(vimState, vimState.cursorStartPosition.getLineBegin(), vimState.cursorPosition.getLineEndIncludingEOL());
        });
    }
};
...
let ActionOverrideCmdD = class ActionOverrideCmdD extends BaseCommand {
    ...
    exec(position, vimState) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!configuration_1.Configuration.disableAnnoyingGcMessage) {
                vscode.window.showInformationMessage("gc is now commentOperator. gb is now 'add new cursor'", "Never show again").then((result) => {
                    if (result === "Never show again") {
                        vscode.workspace.getConfiguration("vim").update("disableAnnoyingGcMessage", true, true);
                        configuration_1.Configuration.disableAnnoyingGcMessage = true;
                    }
                });
            }
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            vscode.window.activeTextEditor.selections =
                vscode.window.activeTextEditor.selections.map((x, idx) => {
                    ...
                    if (idx === 0) {
                        return new vscode.Selection(curPos.getWordLeft(false), curPos.getLeft().getCurrentWordEnd(true).getRight());
                    }
                    else {
