...
function forwardDocumentChanges(server) {
    return vscode_1.workspace.onDidChangeTextDocument(function (event) {
        ...
        serverUtils.updateBuffer(server, { Buffer: document.getText(), Filename: document.fileName }).catch(function (err) {
            console.error(err);
            return err;
        });
    });
...
function forwardFileChanges(server) {
    function onFileSystemEvent(uri) {
        ...
        serverUtils.filesChanged(server, [req]).catch(function (err) {
